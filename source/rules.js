...
export const jsBefore = {
	...
	rules: {
		...
		// eslint-config-bevry/after-typescript-recommended will override this to ignore, as typescript handles it better
		...
		// eslint/recommended will override this to allowing for while(true) loops
		'no-constant-condition': [ERROR, { checkLoops: false }],

		// https://eslint.org/docs/latest/rules/no-control-regex
		// eslint/recommended will override this to error, which is seems like good idea to error about this
		...
		// eslint/recommended will override this to error on duplicate arguments in a function as it can cause errors
		// eslint-config-bevry/after-typescript-recommended will override this to ignore, as typescript handles it better
		'no-dupe-args': ERROR,
		...
		// eslint/recommended will override this to error on duplicate keys in an object as it can cause errors
		// eslint-config-bevry/after-typescript-recommended will override this to ignore, as typescript handles it better
		'no-dupe-keys': ERROR,

		// https://eslint.org/docs/latest/rules/no-duplicate-case
		// eslint/recommended will override this to error on duplicate case statements in a switch
		'no-duplicate-case': ERROR,

		// https://eslint.org/docs/latest/rules/no-empty-character-class
		// eslint/recommended will override this to error on empty [] in regular expressions as they cause unexpected behaviour
		...
		// eslint/recommended will override this to error on overwriting the the exception argument in a catch statement as it can cause memory leaks in some browsers
		'no-ex-assign': ERROR,

		// https://eslint.org/docs/latest/rules/no-extra-boolean-cast
		// eslint/recommended will override this to error on superfluous boolean casts, they offer no value
...
export const jsAfter = {
	...
	rules: {
		...
		// eslint/recommended defaulted this to error
		// however we just want to warn about debugger statements
		'no-debugger': WARN,

		// https://eslint.org/docs/latest/rules/no-empty
		// eslint/recommended defaulted this to error on empty block statements,
		...
		// --------------------------------------------------------------------
		// typescript-eslint
		// https://github.com/typescript-eslint/typescript-eslint/blob/main/packages/eslint-plugin/src/configs/eslintrc/
		...
		// typescript-eslint/stylistic-type-checked will override this to ignore
		'dot-notation': ERROR,

		// https://eslint.org/docs/latest/rules/no-array-constructor
		// Don't use the array constructor when it is not needed
		// typescript-eslint/strict-type-checked will override this to ignore
		...
		// A sneaky way to do eval
		// typescript-eslint/strict-type-checked will override this to ignore
		'no-implied-eval': ERROR,

		// no-redeclare handled earlier
		...
		// typescript-eslint does not modify this, however typescript does not need this
		// eslint-config-bevry/after-typescript-recommended will override this to ignore
		'no-use-before-define': ERROR,

		// https://eslint.org/docs/latest/rules/no-var
		// Of course for non ES5 source code
		// typescript-eslint/eslint-recommended-raw overrides this to error
		// eslint-config-bevry/eslint-five overrides this to ignore
		'no-var': WARN,

		// https://eslint.org/docs/latest/rules/prefer-rest-params
		// Makes sense to enforce, exceptions should be opted out of on case by case
		// eslint-config-bevry/after-typescript-recommended will override this to error, as typescript handles it better
		// eslint-config-bevry/eslint-five overrides this to ignore
		...
		// eslint-config-bevry/eslint-five overrides this to error with config
		...
		// Of course
		// typescript-eslint/eslint-recommended-raw enforces this
		// eslint-config-bevry/after-typescript-recommended will override this to error, as typescript handles it better
		'prefer-const': ERROR,
...
export const nodeAfter = {
	...
	rules: {
		...
		// Seems too difficult to enforce
...
export const typescriptAfter = {
	...
	rules: {
		...
		// || does a different things, and if we use one or the other, it is intentional
		'@typescript-eslint/prefer-nullish-coalescing': IGNORE,

		// it's broken for bevry/editions
		'@typescript-eslint/no-unnecessary-type-parameters': IGNORE,

		// it's broken for bevry/editions, and makes checking type-unsafe inputs infuriating
		'@typescript-eslint/no-unnecessary-condition': IGNORE,

		// this rule is faulty, it wants:
		// `const lowestVersion: string = (broadenedRangeMatch && broadenedRangeMatch[1]) || ''`
		// to become:
		// `const lowestVersion: string = broadenedRangeMatch?[1] || ''`
		// which for some reason is invalid syntax
		'@typescript-eslint/prefer-optional-chain': IGNORE,

		// it's infuriating as obviously a non-string variable is type-cast into a string, we don't have to wrap in String(var) to know that
		'@typescript-eslint/restrict-template-expressions': IGNORE,

		// it's infuriating as anytime we do this, it will be because the object has its own internal string conversion
		'@typescript-eslint/no-base-to-string': IGNORE,
	},
}
